...
- Если запись не найдена, пытается создать её. В случае гонки уникальности (`IntegrityError`) на этапе `commit()` выполняет `rollback()`, повторно считывает запись по тем же полям и возвращает уже созданный конкурентным запросом объект с флагом `False`.
